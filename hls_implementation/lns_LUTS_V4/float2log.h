...
void CCS_BLOCK(LUT_float2log)(ac_fixed< W, I, true> &float_num, ac_fixed< W, I, true> &float_num_out){
...
template< int W, int I >
void CCS_BLOCK(float2log)(ac_fixed< W, I, true> &float_num , ac_fixed< W, I, true> &float_num_out1){
    ...
    LUT_float2log<W,I>(float_num, float_num_temp_out);
    //std::cout << "after LUT " << float_num << "\n";


    // Insert the new integer part
    if(one_found == true){
        float_num_out1 = float_num_temp_out + ac_fixed<W, I, false>(one_place);
    }
	else{
		float_num_out1 = float_num_temp_out;
	}
}

...
void binary2log(int size1, int frac_start1, int* binaryNum1, int* size2, int* frac_start2, int* binaryNum2) {
...
float generateRandomFloat(float maxValue = 20000.0) {
    ...
    std::uniform_real_distribution<float> dist(0.0, maxValue);
...
int main(int argc, char* argv[]) {
    ...
    int frac_start1, frac_start2; // To store where the fractional part starts
    int size1;
    int size2;
    double answer;
    double correct;
    ...
    binary2log(size1, frac_start1,  &binaryNum1[0], &size2, &frac_start2,  &binaryNum2[0]);

    std::cout << "The final anwer is: ";
    printArray(binaryNum2, size2, frac_start2);
    
    binary2double(&size2, &frac_start2,  &binaryNum2[0], &answer);

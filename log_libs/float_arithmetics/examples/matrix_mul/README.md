...
This example provides one implementation of matrix-matrix multiplications using the float_arithmetics library. 

> matrix_mul_dot.cpp    : Contains an implementation using the dot product implementation. 

The implementations is based on the float_arithmetics library. The templatized C++ functions are implemented for HLS using the Catapult HLS tool. The sizes of the two input matrices to be multiplied can be selected through the template parameters ''N'', ''M'' and ''K''. 
